...
- **F2-26 First charts (shipped):** analytics live under `Application/Features/Analytics/` (`GetBurndownQuery`, `GetVelocityQuery`, `GetEpicProgressQuery`) + `AnalyticsController` (`api/v1/projects/{id}/analytics/{burndown,velocity,epic-progress}`). Burndown math is the pure, unit-tested `BurndownCalculator` and charts against the sprint **scope baseline** using `TaskStatusChange` history. Charts are token-styled SVG/CSS (`BurndownChart`, `VelocityChart` + rolling-avg overlay, `EpicProgressBars`) — **not recharts** (removed from `package.json` in the pre-Phase-3 polish pass; SVG is the committed direction). The Engineering dashboard and Sprint detail consume these endpoints. **The pre-Phase-3 polish pass (A + B) killed every dashboard placeholder** — project health (F3-16), KPI strip, team workload (F3-14), and the weekly insight (F3-18, a deterministic unit-tested `WeeklyInsightCalculator` behind `…/analytics/insight`) all render real, server-computed analytics; no `sample` chip remains. **Polish (C) shipped portfolio-level analytics (F3-19):** `GET /api/v1/orgs/{slug}/portfolio` → `GetOrgPortfolioQuery` aggregates every project into per-project health (the type-agnostic, unit-tested `PortfolioHealthCalculator`) + a type-appropriate headline metric, an org rollup strip, and the cross-project resource-conflict list (members split across 2+ projects). `OrgPortfolioPage` consumes it; the old F1.5-08 per-card N+1 `components/portfolio/` summaries were removed. **Polish (D) shipped per-type dashboard depth:** each typed dashboard now reads one server-computed aggregate — `GET …/analytics/{sales,support,marketing,operations}` → `GetSalesAnalyticsQuery` / `GetSupportAnalyticsQuery` / `GetMarketingAnalyticsQuery` / `GetOperationsAnalyticsQuery` — instead of deriving metrics client-side from the list endpoints. New depth metrics: sales weighted forecast + funnel conversion + recent win-rate (`SalesFunnelCalculator`); support SLA attainment over the trailing 30 days (`SlaAttainmentCalculator`); marketing weekly publish throughput sparkline (`ThroughputCalculator`); ops completion / on-time / skip rates over a 30-day window (`RunCompletionCalculator`). All four calculators are pure + unit-tested (`TypedAnalyticsCalculatorTests`). `TypedDashboard` fetches the aggregate once via the registry's `load(projectId)` and passes `{ project, data, loading }` to each widget (one request per dashboard, not one per widget); Generic keeps its self-fetching widgets. **Next analytics gap: typed *roadmap*/AI-inbox adapters** (F1.5-08 left those type-aware too) — the dashboards are now fully real.
